 ...
 * Implements structured schemas and semantic CRM field normalization.
 */
export class GeminiProvider implements LLMProvider {
  ...
  constructor() {
    ...
    logger.info({ provider: this.name, model: this.model }, "Gemini Provider initialized");
  }
  ...
  async extractCrmRecords(
    ...
    logger.info(
      ...
      "Gemini extractCrmRecords started"
    );

    const systemPrompt = `You are a data extraction assistant. Parse raw CSV rows and normalize them into structured CRM records.
You MUST map fields semantically. (e.g. "Buyer", "Client", "Customer", "Contact Name" -> "name"; "Mobile", "Contact No" -> "mobile_without_country_code").
Never map purely by names. Extract context semantically.
...
- crm_status: Allowed values: "GOOD_LEAD_FOLLOW_UP", "DID_NOT_CONNECT", "BAD_LEAD", "SALE_DONE", or "".
- crm_note: Concatenated details or secondary emails/phones.
- data_source: Source of lead.
- possession_time: Timestamp or duration context.
- description: Bio, summary, or description.
...
3. Extract the first email found as primary 'email'. Append any secondary emails to 'crm_note'.
      ...
      try {
        ...
        if (!validated.success) {
          logger.warn(
            ...
            "AI extraction batch failed validation (Gemini)"
          );
          if (retries === 0) {
        ...
        logger.info(
          ...
          "Gemini batch processing completed successfully"
        );
        ...
        logger.error(
          ...
          "Gemini provider extraction failure"
        );
        if (retries === 0) {
